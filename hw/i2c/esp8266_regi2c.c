...
static uint64_t esp8266_i2c_read(void * opaque, hwaddr addr, unsigned int size)
{
    ...
    case A_HOST_0 ... A_HOST_0 + 4 * (ESP8266_REGI2C_HOSTS - 1):
        ...
        res = FIELD_DP32(res, HOST_0, TRANSMISSION_DONE, s->trans_done[(addr - A_HOST_0) / 4]);
        break;
    default:
        qemu_log_mask(LOG_UNIMP, "%s: Unimplemented register read 0x%" HWADDR_PRIx "\n", __func__, addr);
        break;
    }
...
static void esp8266_i2c_write(void * opaque, hwaddr addr, uint64_t value, unsigned int size)
{
    Esp8266RegI2CState * s = ESP8266_REGI2C(opaque);
    ...
    case A_HOST_0 ... A_HOST_0 + 4 * (ESP8266_REGI2C_HOSTS - 1):
        ...
        if (!FIELD_EX32(value, HOST_0, WRITE)) {
            ...
            trace_esp8266_i2c_recive(s->block[(addr - A_HOST_0) / 4], s->reg[(addr - A_HOST_0) / 4]);
        } else {
            i2c_start_send(s->bus, s->block[(addr - A_HOST_0) / 4]);
            i2c_send(s->bus, s->reg[(addr - A_HOST_0) / 4]);
            i2c_send(s->bus, s->data[(addr - A_HOST_0) / 4]);
            i2c_end_transfer(s->bus);
            trace_esp8266_i2c_send(s->block[(addr - A_HOST_0) / 4], s->reg[(addr - A_HOST_0) / 4], s->data[(addr - A_HOST_0) / 4]);
        }
...
static void esp8266_i2c_init(Object * obj)
{
    ...
    memory_region_init_io(&s->iomem, obj, &esp8266_i2c_ops, s, TYPE_ESP8266_REGI2C, ESP8266_REGI2C_MEM_SIZE);
