/*
 * ESP8266 DPORT: Here lays some registers and the MMU for the SPI
...
/*
 * This zone is very undocumented. We can asume that the ESP8266 has a very
 * similar MMU to the ESP32, so somewhere in the dport space we can find the MMU
 * pages status. We also dont know how the illegal access are handled. For now,
 * we just read and write from the flash memory without implementing a more
 * acurrate MMU. More info:
 * https://arduino-esp8266.readthedocs.io/en/latest/mmu.html#
 *
 */
// TODO: More accurated MMU: Cache data sync(MMU pages status, cache flush,
// cache invalidate, etc) and illegal access handling.
...
static uint64_t esp8266_dport_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    case A_DPORT_EDGE_INT_ENABLE:
        ...
        ret = FIELD_DP32(ret, DPORT_EDGE_INT_ENABLE, TIMER1,
                         s->timer1_irq_enabled);
        return ret;
    ...
    case A_DPORT_SPI_INTERRUPT_TYPE:
        ...
        ret = FIELD_DP32(ret, DPORT_SPI_INTERRUPT_TYPE, I2S,
                         0); // Not implemented
        return ret;
    ...
    default:
        qemu_log_mask(LOG_UNIMP,
                      "%s: Unimplemented DPORT register read: 0x%" HWADDR_PRIx
                      "\n",
                      __func__, addr);
        return 0;
    }
                                ...
                                unsigned int size)
{
    ...
    case A_DPORT_EDGE_INT_ENABLE:
        ...
        s->timer1_irq_enabled =
            FIELD_EX32(value, DPORT_EDGE_INT_ENABLE, TIMER1);
        break;
    case A_DPORT_SPI_CACHE:
        ...
        if (s->cache.enabled) {
            ...
            qemu_log("%s:    is 2MB size: %d\n", __func__,
                     s->cache.is_2MB_block);
            qemu_log("%s:    is 1 MB offset: %d\n", __func__,
                     s->cache.is_1MB_offset);
        }

        // memory_region_set_enabled(&s->cache->flash_cache.mem,
        // s->cache->enabled);

        // Throw Unimplement
        if (FIELD_EX32(value, DPORT_SPI_CACHE, FLUSH_START)) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: Unimplemented Cache flush operation" HWADDR_PRIx
                          "\n",
                          __func__);
        }
        if (FIELD_EX32(value, DPORT_SPI_CACHE, EMPTY)) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: Unimplemented Cache empty operation" HWADDR_PRIx
                          "\n",
                          __func__);
        }
        if (FIELD_EX32(value, DPORT_SPI_CACHE, BUSY)) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: Unimplemented Cache busy operation" HWADDR_PRIx
                          "\n",
                          __func__);
        }
        if (FIELD_EX32(value, DPORT_SPI_CACHE, TARGET)) {
            qemu_log_mask(LOG_UNIMP,
                          "%s: Unimplemented Cache target operation" HWADDR_PRIx
                          "\n",
                          __func__);
        }
        break;
...
{
    // Based on:
    // https://github.com/espressif/ESP8266_RTOS_SDK/blob/d45071563cebe9ca520cbed2537dc840b4d6a1e6/components/spi_flash/src/spi_flash.c#L754
...
{
    Esp8266CacheRegionState *s = opaque;
    uint64_t ret = 0;
    if (s->cache->enabled) {
...
static void esp8266_cache_write(void *opaque, hwaddr addr, uint64_t value,
                                unsigned int size)
{
    Esp8266CacheRegionState *s = opaque;
    if (s->cache->enabled) {
        uint32_t spi_addr = esp8266_cache_to_spi_addr(s->cache, addr);
        blk_pwrite(s->cache->dport->flash_blk, spi_addr, &value, size,
                   0); // TODO: Flags
    } else {
        error_report("%s: Cache not enabled. (Addr: 0x%" HWADDR_PRIx ")\n",
                     __func__, addr);
...
{
    crs->base = base;
    memory_region_init_io(&crs->mem, OBJECT(cs->dport), &esp8266_cache_ops, crs,
                          "flash-cache", ESP8266_CACHE_SIZE);
}
...
static void esp8266_dport_reset(DeviceState *dev)
{
    ...
    // Based on
    // https://github.com/esp8266/Arduino/blob/da6ec83b5fdbd5b02f04cf143dcf8e158a8cfd36/doc/rgisters_dump.txt
    s->wdt_irq_enabled = 1;
...
static void esp8266_dport_init(Object *obj)
{
    ...
    esp8266_cache_init_region(&s->cache, &s->cache.flash_cache, "flash-cache",
                              ESP8266_CACHE_BASE);
}
